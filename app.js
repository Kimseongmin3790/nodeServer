...
app.get('/board/list', async (req, res) => {
  ...
  let query1 = `SELECT B.*, TO_CHAR(CDATETIME, 'YYYY-MM-DD') CTIME FROM TBL_BOARD B` + subQuery;
  try {
...
app.get('/board/update', async (req, res) => {
  const { title, contents, userId, kind, boardNo } = req.query;
  let query = `UPDATE TBL_BOARD ` +
            `SET TITLE = '${title}', CONTENTS = '${contents}', USERID = '${userId}', KIND = ${kind} WHERE BOARDNO = ${boardNo}`;
  try {
    await connection.execute(
      query,
      [],
